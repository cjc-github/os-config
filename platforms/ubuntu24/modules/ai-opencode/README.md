...
通过 npm 全局安装 `opencode-ai` 的固定版本（OpenCode CLI）。

> **注意**：npm 包名是 `opencode-ai`（**不是** `opencode`），极易写错。命令行可执行名仍是 `opencode`。
...
3. **幂等检查**：`cmd_exists opencode` 且解析出的版本号 == `OPENCODE_VERSION` → 跳过（`--force` 时继续重装）
4. **安装**：`npm install -g $OPENCODE_PKG@$OPENCODE_VERSION`（`with_retry` 重试 3 次，npm install -g 会覆盖现有版本）

## 验证项

- `opencode` 命令可用（解析打印版本号）
- 实际版本 == `OPENCODE_VERSION`（不一致仅告警，不视为失败）
...
实查最新稳定版：`npm view opencode-ai version`

## 幂等行为

- 安装型：`cmd_exists opencode` 且解析版本 == `OPENCODE_VERSION` → 跳过
...
```bash
# 只跑本模块（自动补依赖）
./setup.sh --module opencode

# 跑全部
./setup.sh

# 强制重装
./setup.sh --module opencode --force

# 只验证
./setup.sh --module opencode --verify-only
```
