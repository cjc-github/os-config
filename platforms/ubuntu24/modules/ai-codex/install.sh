#!/usr/bin/env bash
# =============================================================================
# 模块：ai-codex  ——  安装 OpenAI Codex CLI
# 平台：ubuntu24
# 作用：通过 npm 全局安装 @openai/codex 的固定版本
...
# ---------------------------------------------------------------------------
# 1) 幂等检查：codex 已存在且版本一致 → 跳过
# ---------------------------------------------------------------------------
if cmd_exists codex && [[ "${FORCE:-0}" != 1 ]]; then
  cur=$(codex --version 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "")
  if [[ "$cur" == "$CODEX_VERSION" ]]; then
    log_info "codex $cur 已安装，跳过（用 --force 或 -f 强制重装）"
    exit 0
  fi
fi
...
if [[ -n "$prefix" ]]; then
  ...
  if [[ -d "$nm_dir/$CODEX_PKG" ]]; then
    log_info "清理旧目录：$nm_dir/$CODEX_PKG"
    rm -rf "$nm_dir/$CODEX_PKG" 2>/dev/null || \
      log_warn "清理失败（可能权限不足），继续尝试 npm install"
  fi
  pkg_base="${CODEX_PKG##*/}"
  shopt -s nullglob
  for d in "$nm_dir"/."$pkg_base"-* "$nm_dir"/."$CODEX_PKG"-*; do
    [[ -d "$d" ]] && {
...
# ---------------------------------------------------------------------------
run_step_verbose "npm install -g $CODEX_PKG@$CODEX_VERSION (registry=$NPM_REGISTRY)" \
