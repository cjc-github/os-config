...
通过 npm 全局安装 `@anthropic-ai/claude-code` 的固定版本（Claude Code CLI）。
...
3. **幂等检查**：`cmd_exists claude` 且解析出的版本号 == `CLAUDE_CODE_VERSION` → 跳过（`--force` 时继续重装）
4. **安装**：`npm install -g $CLAUDE_CODE_PKG@$CLAUDE_CODE_VERSION`（`with_retry` 重试 3 次，npm install -g 会覆盖现有版本）

## 验证项

- `claude` 命令可用（解析打印版本号）
- 实际版本 == `CLAUDE_CODE_VERSION`（不一致仅告警，不视为失败）
...
实查最新稳定版：`npm view @anthropic-ai/claude-code version`

## 幂等行为

- 安装型：`cmd_exists claude` 且解析版本 == `CLAUDE_CODE_VERSION` → 跳过
