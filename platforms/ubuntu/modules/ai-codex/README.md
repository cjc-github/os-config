...
通过 npm 全局安装固定版本的 OpenAI Codex CLI（`@openai/codex`）。
...
2. 若 `codex --version` 已等于 `${CODEX_VERSION}`，且没有 `--force`，则幂等跳过。
3. 首次正常执行 `npm install -g ${CODEX_PKG}@${CODEX_VERSION}`，使用配置的 `NPM_REGISTRY`。
