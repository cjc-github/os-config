...
通过 npm 全局安装固定版本的 OpenCode CLI（`opencode-ai`）。

> npm 包名是 `opencode-ai`，可执行命令是 `opencode`。
...
2. 若 `opencode --version` 已等于 `${OPENCODE_VERSION}`，且没有 `--force`，则幂等跳过。
3. 首次正常执行 `npm install -g ${OPENCODE_PKG}@${OPENCODE_VERSION}`，使用配置的 `NPM_REGISTRY`。
...
| 变量 | 当前默认值 | 说明 |
|---|---|---|
| `OPENCODE_PKG` | `opencode-ai` | npm 包名 |
| `OPENCODE_VERSION` | `1.18.18` | 固定版本号 |
